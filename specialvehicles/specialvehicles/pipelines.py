...
class PostgresqlPipeline:
    ...
    def process_item(self, item: dict, spider) -> dict:
        try:
            ...
            check_query = "SELECT EXISTS (SELECT 1 FROM specialvehicles_olx WHERE content_id = %s)"
            ...
            query = """
               insert into specialvehicles_olx (title, description, content_id, price, currency, main_category, category, category_type, url, isBusiness, isHighlighted, isPromoted, promotion, delivery, createdTime, lastRefreshTime, pushupTime, validToTime, isActive, status, isJob, itemCondition, negotiable, cityName, regionName, districtName, olx_user)
