...
class AgromachineSpider(scrapy.Spider):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.page_item_count = {}
    name = "agromachinespiders"
    allowed_domains = ["www.olx.uz"]
    start_urls = [
        'https://www.olx.uz/transport/selhoztehnika/?currency=UZS&search%5Bprivate_business%5D=business',
        'https://www.olx.uz/transport/selhoztehnika/?currency=UZS&search%5Bprivate_business%5D=private'
    ]
    ...
    custom_settings = {
        ...
        "FEEDS": {
            "agromachine_olx.json": {"format": "json", "overwrite": True}
        }
    ...
    # Extract links to listing detail pages
        for agromachine in response.css('div[data-cy="l-card"]'):
            detail_page = agromachine.css('div[data-cy="ad-card-title"]>a::attr(href)').get()
            if detail_page:
